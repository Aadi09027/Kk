...
try:
    ...
    os.chmod(aadi_path, 0o755)  # 755 grants execute permissions for all users

    print(f"Execute permission granted for {aadi_path}.")
...
def handle_aadi(message):
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in aadi_cooldown and (datetime.datetime.now() - aadi_cooldown[user_id]).seconds < 60:
        ...
        if len(command) == 4:  # Updated to accept target, port, and time
            ...
            if time > 151:
                response = "ᴇʀʀᴏʀ: ᴍᴀx ᴀᴛᴛᴀᴄᴋ sᴇᴄᴏɴᴅ 150sᴇᴄ ❌."
            else:
...
def welcome_plan(message):
    user_name = message.from_user.first_name
    response = f'''{user_name}, ʙᴜʏ ғʀᴏᴍ @user_x_dead
